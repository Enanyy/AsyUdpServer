<<<<<<< 68df443d39bfed30cb9f48a010c78c56448ce646
#if defined( _MSC_VER )
...
bool XMLTest (const char testString, const char expected, const char found, bool echo=true, bool extraNL=false )
{
	...
	if ( !echo ) {
		...
		if ( extraNL ) {
			...
			printf ( %s [%s][%s]n, testString, expected, found);
		}
	}
...
{
    return XMLTest(testString, XMLDocumentErrorIDToName(expected), XMLDocumentErrorIDToName(found), echo, extraNL);
}
...
template class T  bool XMLTest( const char testString, T expected, T found, bool echo=true )
{
	...
	if ( !echo )
		...
		printf ( %s [%d][%d]n, testString, static_castint(expected), static_castint(found) );
...
{
	XMLDocument doc;
	doc.LoadFile( resourcesdream.xml );
...
int example_3()
{
	static const char xml =
		...
		PLAY
		TITLEA Midsummer Night's DreamTITLE
		PLAY;
 ...
 @page Example-3 Get information out of XML
	...
	ul
		...
		ul
			liElement TITLEli
			ul
			    liText A Midsummer Night's Dreamli
			ul
		ul
	ul
	...
	@verbatim
	Consider A Midsummer Night's bDreamb
	@endverbatim
...
bool example_4()
{
	...
	printf( Both values are the same %d and %dn, v0, v1 );
...
int main( int argc, const char  argv )
{
	...
	if ( argc  1 ) {
		...
		printf( Test file '%s' loaded. ErrorID=%dn, argv[1], errorID );
		if ( !errorID ) {
			printf( Load time=%un,   (unsigned)(loadTime - startTime) );
			printf( Delete time=%un, (unsigned)(deleteTime - loadTime) );
			printf( Total time=%un,  (unsigned)(deleteTime - startTime) );
		}
		exit(0);
	}

	FILE fp = fopen( resourcesdream.xml, r );
	if ( !fp ) {
		printf( Error opening test file 'dream.xml'.n
				Is your working directory the same as where n
				the xmltest.cpp and dream.xml file arenn
	#if defined( _MSC_VER )
				In windows Visual Studio you may need to setn
				Properties-Debugging-Working Directory to '..'n
	#endif
			  );
		exit( 1 );
	}
...

	{
		static const char test[] = {	element ,
										elementelement,
										elementsubelementelement,
										elementsubelementsubelementelement,
										elementsubelementsubsubsubelementelement,
										!--comment beside elements--elementsubelementsubelementelement,
										!--comment beside elements, this time with spaces--  n element  subelement n subelement element,
										element attrib1='foo' attrib2=bar element,
										element attrib1='foo' attrib2=bar subelement attrib3='yeehaa' element,
										elementText inside element.element,
										elementbbelement,
										elementText inside and bboldedb in the element.element,
										outerelementText inside and bboldedb in the element.elementouter,
										elementThis &amp; That.element,
...
#if 1
	{
		static const char test = !--hello worldn
								            line 2r
								            line 3rn
								            line 4nr
								            line 5r--;
	...
	{
		static const char test = elementText before.element;
		XMLDocument doc;
		doc.Parse( test );
	...
	{
		XMLDocument doc = new XMLDocument();
		static const char test = elementsubelement;
		doc-Parse( test );
	...
	}
	{
		...
		}
		{
			...
			doc-Print( &streamer );
			XMLTest( Compact mode, elementsub attrib=truesubelement, streamer.CStr(), false );
		}
		doc-SaveFile( .resourcesoutpretty.xml );
		XMLTest( Save pretty.xml, false, doc-Error() );
		doc-SaveFile( .resourcesoutcompact.xml, true );
	...
	}
	{
		...
		doc.SaveFile( resourcesoutdreamout.xml );
		...
		XMLDocument doc2;
		doc2.LoadFile( resourcesoutdreamout.xml );
	...
	{
		const char error =	xml version=1.0 standalone=no n
							passages count=006 formatversion=20020620n
							    wrong errorn
							passages;
	...
	{
		XMLDocument doc;
		doc.LoadFile( resourcesutf8test.xml );
		...
		const unsigned char russianElementName[] = {	0xd0U, 0xa0U, 0xd1U, 0x83U,
														...
														0xd0U, 0xb9U, 0 };
		const char russianText[] = xD0xB8xD0xBCxD0xB5xD0xB5xD1x82;
		...
		doc.SaveFile( resourcesoututf8testout.xml );
		XMLTest( UTF-8 Save testout.xml, false, doc.Error() );

		 Check the round trip.
		bool roundTripOkay = false;

		FILE saved  = fopen( resourcesoututf8testout.xml, r );
		XMLTest( UTF-8 Open utf8testout.xml, true, saved != 0 );

		FILE verify = fopen( resourcesutf8testverify.xml, r );
		...
		if ( saved && verify )
		{
			...
			while ( fgets( verifyBuf, 256, verify ) )
			{
				...
				{
					printf( verify%sn, verifyBuf );
					printf( saved %sn, savedBuf );
					roundTripOkay = false;
					break;
				}
			}
	 ...
	 --------GetText()-----------
	{
		const char str = fooThis is  textfoo;
		XMLDocument doc;
		doc.Parse( str );
	 ...
	 --------SetText()-----------
	{
		...
		XMLTest( SetText() normal use (openclose)., darkness., element-GetText() );
		...
		element-SetText(bhorsesb);
		XMLTest( SetText() replace with tag-like text., bhorsesb, element-GetText() );
		doc.Print();

		str = foobarText in nested elementbarfoo;
		doc.Parse( str );
	 ...
	 ---------- Attributes ---------
	{
		...
		}
		{
			...
			XMLUtilSetBoolSerialization(1, 0);
			element-SetAttribute(attrib, true);
			result = element-Attribute(attrib);
			XMLTest(Bool true is '1', 1, result);

			XMLUtilSetBoolSerialization(0, 0);
		}
		{
	...
	{
		{
			FILE printerfp = fopen(resourcesoutprinter.xml, w);
		...
		{
			XMLDocument doc;
			doc.LoadFile(resourcesoutprinter.xml);
	 ...
	 ---------- CDATA ---------------
	{
		...
		XMLTest( CDATA parse., I am  the rules!n...since I make symbolic puns,
	 ...
	 ----------- CDATA -------------
	{
		...
		XMLTest( CDATA parse. [ tixml11480107 ],
								 bI am  the rules!bn...since I make symbolic puns,
								 doc.FirstChildElement()-FirstChild()-Value(),
								 false );
	}
...

	{
		...
		const char expected = Line 5 has quotation marks and 'apostrophe marks'. It also has , , and &, as well as a fake copyright xC2xA9.;

		XMLTest( Entity transformation read. , expected, context, true );

		const char textFilePath = resourcesouttextfile.txt;
		...
		if ( textfile )
		{
			...
			XMLTest( Entity transformation write. ,
					 psg context=Line 5 has &quot;quotation marks&quot; and &apos;apostrophe marks&apos;.
					  It also has &lt;, &gt;, and &amp;, as well as a fake copyright xC2xA9.n,
					 buf, false );
			fclose( textfile );
		}
	}
	...
	{
		const char test = elementName1.1 Start easy ignore fin thickness&#xA;Nameelement;
		...
		XMLTest( Entity with one digit.,
				 1.1 Start easy ignore fin thicknessn, text-Value(),
				 false );
	}
...

	{
		...
		XMLTest( Comment formatting.,  Somewhatevil , comment-Value() );
	}
	{
		 Double attributes
...

	{
		...
		const char doctype = element att0r='red' attr='blue' ;
...

	{
		...
		unsigned char buf[] = xml version=1.0 encoding=utf-8feed![CDATA[Test XMLblablablalblbl;
		buf[60] = 239;
		buf[61] = 0;
...

	{
		...
		XMLDocument xml;
		xml.Parse( infiniteloop );
		XMLTest( No closing element, true, xml.Error() );
		XMLTest( Infinite loop test., true, true );
	}
#endif
	{
		const char pub = xml version='1.0' elementsubelement !--comment-- !DOCTYPE;
		XMLDocument doc;
		doc.Parse( pub );
...

	{
		...
		static const char xml = docelement attribute='attribute'element attribute='attribute'doc;
		XMLDocument doc;
		doc.Parse( xml );
	 ...
	 -------- Handles ------------
	{
		static const char xml = element attrib='bar'subTextsubelement;
		XMLDocument doc;
		doc.Parse( xml );
	...
	}
	{
		...
		static const char result  = xefxbbxbfxml version=1.0 encoding=UTF-8;
	...
	}
	{
	 ...
	 QueryXYZText
	{
		const char xml = point x1.2x y1y z38z validtruevalid point;
		XMLDocument doc;
		doc.Parse( xml );
	...
	{
		const char xml = element_subsubsubsubsub-subelement;
		XMLDocument doc;
		doc.Parse( xml );
    ...
    {
        const char xml = element _attr1=foo attr2=barelement;
        XMLDocument doc;
        doc.Parse( xml );
	...
	{
		const char xml = elementWOA THIS ISN'T GOING TO PARSE;
		XMLDocument doc;
		doc.Parse( xml, 10 );
	...
	{
		const char xml = parentchildabcchildparent;
		XMLDocument doc;
		doc.Parse( xml );
		...
		XMLTest( Printing of sub-element, childabcchildn, printer.CStr(), false );
	}


	{
		XMLDocument doc;
		XMLError error = doc.LoadFile( resourcesempty.xml );
	...
	{
         BOM preservation
        static const char xml_bom_preservation  = xefxbbxbfelementn;
        {
			XMLDocument doc;
			...
			doc.SaveFile( resourcesbomtest.xml );
			XMLTest( Save bomtest.xml, false, doc.Error() );
        }
		{
			XMLDocument doc;
			doc.LoadFile( resourcesbomtest.xml );
...

	{
		...
		const char xml = xml version=1.0rootsamplefield01field0field12field1sampleroot;
		XMLDocument doc;
		doc.Parse(xml);
		...
		doc.PrintError();
		 ...
		 httpsgithub.comleethomasontinyxml2issues63
	{
		...
		XMLTest( Float and double formatting., 
			element attrA-f64=123456789.12345679 attrB-f64=1001000000 attrC-f64=1e+20 attrD-f64=0.123456789 attrA-f32=1.2345679e+08 attrB-f32=1.001e+09 attrC-f32=1e+20 attrD-f32=0.12345679n,
			printer.CStr(), 
			true );
    ...
    
    {
        ...
        const char xmlText = xml version=1.0 encoding=UTF-8test;
        {
            XMLDocument doc;
            doc.Parse(xmlText);
    ...
    
    {
        ...
        tinyxml2XMLDocument doc;
        tinyxml2XMLElement pRoot = doc.NewElement(Root);
	...
	{
		 Issue 302. Clear errors from LoadFileSaveFile
		XMLDocument doc;
		XMLTest( Issue 302. Should be no error initially, XML_SUCCESS, doc.ErrorName() );
		doc.SaveFile( .nosuchpathpretty.xml );
...

	{
		...
		doc.LoadFile( resourcesno-such-file.xml );
...

	{
	    ...
	    const char xml1 = xml version=1.0 
	                       xml-stylesheet type=textxsl href=Anything.xsl
	                       first ;
	    const char xml2 = first 
	                       xml version=1.0 ;
	    const char xml3 = firstfirst
	                       xml version=1.0 ;

	    const char xml4 = firstxml version=1.0 first;
...

    {
	     ...
	     calling XMLDocumentValue() used to cause an assert in debug.
	     Null must be retured.
	    const char validXml = xml version=1.0 encoding=utf-8 
	                           first 
	                           second ;
	    XMLDocument doc = new XMLDocument();
	    XMLTest( XMLDocumentValue() returns null, NULL, doc-Value() );
	    doc-Parse( validXml );
	    XMLTest( Parse to test XMLDocumentValue(), false, doc-Error());
	    XMLTest( XMLDocumentValue() returns null, NULL, doc-Value() );
	    delete doc;
    }

	{
		...
		for( int i = 0; i  XML_ERROR_COUNT; i++ ) {
			const XMLError error = static_castXMLError(i);
			const char name = XMLDocumentErrorIDToName(error);
...

	{
		...
		const char xml =
			...
			property name='track_name'voiceproperty
			property name='audio_track'1property
     ...
     ----------- Line Number Tracking --------------
    {
		...
		tester.TestParseError(ErrorLine-Parsing, nrootn foo nunclosed, XML_ERROR_PARSING, 2);
        tester.TestParseError(ErrorLine-Declaration, rootnxml version=1.0, XML_ERROR_PARSING_DECLARATION, 2);
        tester.TestParseError(ErrorLine-Mismatch, nrootnmismatch, XML_ERROR_MISMATCHED_ELEMENT, 2);
        tester.TestParseError(ErrorLine-CData, nroot![CDATA[ n foo bar n, XML_ERROR_PARSING_CDATA, 2);
        tester.TestParseError(ErrorLine-Text, nrootn foo bar n, XML_ERROR_PARSING_TEXT, 3);
        tester.TestParseError(ErrorLine-Comment, nrootn!-- n, XML_ERROR_PARSING_COMMENT, 3);
        tester.TestParseError(ErrorLine-Declaration, nrootn n, XML_ERROR_PARSING_DECLARATION, 3);
        tester.TestParseError(ErrorLine-Unknown, nrootn! n, XML_ERROR_PARSING_UNKNOWN, 3);
        tester.TestParseError(ErrorLine-Element, nrootnunclosed n, XML_ERROR_PARSING_ELEMENT, 3);
        tester.TestParseError(ErrorLine-Attribute, nrootnunclosed n attn, XML_ERROR_PARSING_ATTRIBUTE, 4);
        tester.TestParseError(ErrorLine-ElementClose, nrootnunclosed nunexpected, XML_ERROR_PARSING_ELEMENT, 3);

		tester.TestStringLines(
            LineNumbers-String,

            xml version=1.0n					 1 Doc, DecL
                root a='b' n						 2 Element Attribute
                c='d' d blah  n					 3 Attribute Text Element
                newline in text n					 4 Text
                and second zxcv![CDATA[n			 5 Element Text
                 cdata test ]]!-- comment --n		 6 Comment
                ! unknownroot,					 7 Unknown
		...
		tester.TestStringLines(
            ...
            rn										 1 Doc (arguably should be line 2)
            xml version=1.0n					 2 DecL
            rootrn								 3 Element
            n										 4
            text contining new line n				 5 Text
             and also containing crlf rn			 6
            sub![CDATA[n							 7 Element Text
            cdata containing new line n				 8
             and also containing cflrrn				 9
            ]]subsub2root,					 10 Element

            D01L02E03T05E07T07E10);

		tester.TestFileLines(
            LineNumbers-File,
            resourcesutf8test.xml,
            D01L01E02E03A03A03T03E04A04A04T04E05A05A05T05E06A06A06T06E07A07A07T07E08A08A08T08E09T09E10T10);
    }

    {
    	const char xml = HelloTextError;
    	XMLDocument doc;
    	doc.Parse(xml);
     ...
     ----------- Performance tracking --------------
	{
		...
		FILE perfFP = fopen(resourcesdream.xml, r);
...
#endif
		printf(nParsing dream.xml (%s) %.3f milli-secondsn, note, duration);
	}
	...
	printf (nPass %d, Fail %dn, gPass, gFail);
